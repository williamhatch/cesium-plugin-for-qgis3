...
from .q3dcore import MapTo3D, Layer, GDALDEMProvider, FlatDEMProvider, calculateGridSegments, layerTypeFromMapLayer, \
    urlFromPCLayer
...
class ExportSettings:
    ...
    def loadSettingsFromFile(self, filepath=None):
        ...
        if settings.get("Version", 0) < 20700:
            try:
                ...
                logMessage(
                    "ExportSettings: Failed to load some properties which were saved with an earlier plugin version.")
    ...
    def demProviderByLayerId(self, id):
        ...
        if id.startswith("plugin:"):
            ...
            if layer:
                return GDALDEMProvider(layer.source(), str(self.crs.toWkt()),
                                       source_wkt=str(layer.crs().toWkt()))  # use CRS set to the layer in QGIS
    ...
    def demGridSegments(self, layerId):
        ...
        if layer:
            return calculateGridSegments(self.baseExtent(),
                                         ...
                                         layer.properties.get("spinBox_Roughening", 0) if layer.properties.get(
                                             "checkBox_Tiles") else 0)
        return QSize(1, 1)
    ...
    def enabledValidKeyframeGroups(self, layerId=None, warning_log=None):
        ...
        def warn_one_keyframe(group):
            if warning_log:
                warning_log(
                    "'{}' group has only one keyframe. At least two keyframes are necessary for this group to work.".format(
                        group["name"]))
        ...
        if layerId is None:
            ...
            if count > 1 and warning_log:
                warning_log(
                    "There are {} enabled camera motion groups. They may not work properly due to conflicts.".format(
                        count))
                ...
                if group["type"] == ATConst.ITEM_GRP_GROWING_LINE:
                    ...
                    if layer:
                        if layer.properties.get("comboBox_ObjectType") in ["Line", "Thick Line"]:
                            ...
                            warning_log(
                                "Layer '{}': Growing line animation is available with 'Line' and 'Thick Line'.".format(
                                    layer.name))
                    else:
    ...
    def narrations(self, indent=2, indent_width=2):
        ...
        for g in self.enabledValidKeyframeGroups():
            for k in g.get("keyframes", []):
                ...
                html = '{}<div id="{}" class="narcontent">\n{}\n{}</div>'.format(s * indent, nar["id"], content,
                                                                                 s * indent)
                d.append(html)
...
def deepcopyExcept(obj, keys_to_remove):
    if isinstance(obj, dict):
        return {k: deepcopyExcept(v, keys_to_remove) if isinstance(v, (dict, list)) else v for k, v in obj.items() if
                k not in keys_to_remove}
